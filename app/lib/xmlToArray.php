...
//         $data = mb_convert_encoding($data,"UTF-8" , "GB2312, BIG5, ASCII");
        ...
        try {
            xml_parse_into_struct($this->parser, $data, $this->_struct);
        } catch (Exception $e) {
//            Log::write('info','xml_parse_into_struct:'.var_export($data,true));
//            Log::write('info',var_export($e->getMessage(),true));
            $this->free();
            exit();
        }
//         if (!xml_parse_into_struct($this->parser, $data, $this->_struct))
//         {
//             //log_message('error','xml_parse_into_struct:'.var_export($data,true));
